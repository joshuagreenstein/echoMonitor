...
//Run with: node PORT=3003 server.js
...
app.post('/echoData', function(req, res){
  ...
  if (summary.indexOf("lock the front door")!=-1) {
    greenled.writeSync(1);
  }
  if (summary.indexOf("unlock the front door")!=-1) {
    greenled.writeSync(0);
  }
  if (summary.indexOf("lock the back door ")!=-1) {
    redled.writeSync(1);
  }
  if (summary.indexOf("unlock the back door")!=-1) {
    redled.writeSync(0);
d  }
  if (summary.indexOf("unlock all doors")!=-1) {
    redled.writeSync(0);
    greenled.writeSync(0);
    lamp.writeSync(0);
  }
  if (summary.indexOf("lock all doors")!=-1) {
    redled.writeSync(1);
    greenled.writeSync(1);
    lamp.writeSync(1);
  }
